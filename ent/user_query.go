...
type UserQuery struct {
	...
	// eager-loading edges.
	withAttending *SlotQuery
	withVisiting  *SlotQuery
...
// QueryAttending chains the current query on the "Attending" edge.
func (uq *UserQuery) QueryAttending() *SlotQuery {
	query := &SlotQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, user.AttendingTable, user.AttendingColumn),
		)
...
// QueryVisiting chains the current query on the "Visiting" edge.
func (uq *UserQuery) QueryVisiting() *SlotQuery {
	query := &SlotQuery{config: uq.config}
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := uq.prepareQuery(ctx); err != nil {
			return nil, err
		...
		step := sqlgraph.NewStep(
			sqlgraph.From(user.Table, user.FieldID, selector),
			sqlgraph.To(slot.Table, slot.FieldID),
			sqlgraph.Edge(sqlgraph.O2M, false, user.VisitingTable, user.VisitingColumn),
		)
		fromU = sqlgraph.SetNeighbors(uq.driver.Dialect(), step)
		return fromU, nil
	}
...
func (uq *UserQuery) Clone() *UserQuery {
	...
	return &UserQuery{
		...
		withAttending: uq.withAttending.Clone(),
		withVisiting:  uq.withVisiting.Clone(),
...
// WithAttending tells the query-builder to eager-load the nodes that are connected to
// the "Attending" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithAttending(opts ...func(*SlotQuery)) *UserQuery {
	query := &SlotQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withAttending = query
	return uq
}

// WithVisiting tells the query-builder to eager-load the nodes that are connected to
// the "Visiting" edge. The optional arguments are used to configure the query builder of the edge.
func (uq *UserQuery) WithVisiting(opts ...func(*SlotQuery)) *UserQuery {
	query := &SlotQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withVisiting = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withAttending; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Attending = []*Slot{}
		}
		query.withFKs = true
		query.Where(predicate.Slot(func(s *sql.Selector) {
			s.Where(sql.InValues(user.AttendingColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_attending
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_attending" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_attending" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Attending = append(node.Edges.Attending, n)
		}
	}

	if query := uq.withVisiting; query != nil {
		fks := make([]driver.Value, 0, len(nodes))
		nodeids := make(map[int]*User)
		for i := range nodes {
			fks = append(fks, nodes[i].ID)
			nodeids[nodes[i].ID] = nodes[i]
			nodes[i].Edges.Visiting = []*Slot{}
		}
		query.withFKs = true
		query.Where(predicate.Slot(func(s *sql.Selector) {
			s.Where(sql.InValues(user.VisitingColumn, fks...))
		}))
		neighbors, err := query.All(ctx)
		if err != nil {
			return nil, err
		...
		for _, n := range neighbors {
			fk := n.user_visiting
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_visiting" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_visiting" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.Visiting = append(node.Edges.Visiting, n)
		}
	}
